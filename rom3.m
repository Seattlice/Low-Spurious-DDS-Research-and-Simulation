...
for i = 1 : N/4
    ...
    A = bitshift(round(2*pi*F1*i) , -B_weishu);%粗ROM中A和细ROM中B的确定
    A = T4 * A * 4 / N;
    
    B = round(2*pi*F1*i) - 2^(12 - B_weishu) * A;
    B = T4 * B * 4 / N;    
    if(i <= 2^(12 - B_weishu))
        a(i) = sin(A);
    end
...
elseif romaddr_reg <= N/2 && romaddr_reg > N/4
        ...
        rom_cu = rom1(A);
        rom_xi = rom2(romaddr_reg+1);
       dac_data =rom_cu + rom_xi + ADC; 
        fuhao = 2;
    %第1象限    
