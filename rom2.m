...
T = 2 ^ 20;%仿真的实践
%% sin
ss=sin(2*pi*F1*t+pi*P1/180) - 4*F1*t;
%plot(ss);
car =   ss ;
for i = 1:N/4
    if(car(i) < 0)
...
if (romaddr_reg >= 3/4*N) && (romaddr_reg < N)
        romaddr_reg = N - romaddr_reg;
        dac_data =  ADC - car(romaddr_reg) - A*4*F1*4*romaddr_reg*T4/N;
        fuhao = 4;
    %第3象限    
elseif romaddr_reg >= N/2 && romaddr_reg < 3/4*N 
        romaddr_reg = 3/4*N - romaddr_reg;
        dac_data =  ADC -car(N/4 - romaddr_reg + 1 )- A*4*F1*4*(N/4 - romaddr_reg+ 1)*T4/N;
        fuhao = 3;        
    %第2象限    
elseif romaddr_reg < N/2 && romaddr_reg >= N/4
        romaddr_reg =  N/2 - romaddr_reg ;
        dac_data = car( romaddr_reg+1) +  A * 4*F1*4*(romaddr_reg+1 )*T4/N + ADC;
        fuhao = 2;
    %第1象限    
...
%    disp(fuhao);
    s3(i) = dac_data ;
    if i > 2
        if (s3(i-1) >= 0.999999 )&& (s3(i-2) >= 0.999999 )&& (s3(i) < 0.5) 
            s3(i) = 1;
        end
    end
end
